# James Palmer
# Minesweeper, but with a set number of mine arrangements
# November 2023
...
class Minesweeper:
    ...
    def createBoard(self):
        ...
        mineArrangement = random.randint(0,7)
        match mineArrangement:
            # Vertical line | 
            case 0:
            ...
            case 3:
                for i in range(self.size):
                    board[i][self.size - i - 1] = -1
            
            # Square of mines, bottom left corner
            case 4:
                self.mineBox(board, self.size-2, 1)

            # Square of mines, top left corner
            case 5:
                self.mineBox(board, 1, 1)

            # Square of mines, bottom right corner
            case 6:
                self.mineBox(board, self.size-2, self.size-2)

            # Square of mines, top right corner
            case 7:
                self.mineBox(board, 1, self.size-2)
                
        return board

    # Places a 3x3 box of mines in a specified location
    def mineBox(self, board, row, col):
        board[row][col] = -1
        # list of directions for mine placement
